...
```
nama | desa | kec | kamar | harga | kontak | nib | sert | pbg | lat | lng | tanggal_survei
```
- `nib`, `sert`, `pbg` isinya salah satu: `belum`, `proses`, `terbit`
- `lat`, `lng` boleh dikosongkan kalau belum tahu koordinat (unit itu tidak
  akan muncul sebagai titik di peta, tapi tetap muncul di daftar & laporan)
...
```
tag | title | excerpt | date | read | seed
```
